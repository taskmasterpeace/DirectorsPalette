...
const handleSaveProject = async () => {
  ...
  try {
    ...
    if (currentProjectId) {
      ...
      await projectDB.updateProject(currentProjectId, currentProject)
    } else {
      // Create new project
      projectId = await projectDB.saveProject({
        name: currentProject.name || `${currentProject.isMusicVideoMode ? 'Music Video' : 'Story'} Project ${new Date().toLocaleDateString()}`,
        ...currentProject
      })
...
const filteredProjects = projects.filter(project =>
  ...
  (project.musicVideoData?.songTitle && project.musicVideoData.songTitle.toLowerCase().includes(searchQuery.toLowerCase())) ||
  (project.musicVideoData?.artist && project.musicVideoData.artist.toLowerCase().includes(searchQuery.toLowerCase()))
)
...
const getProjectStats = (project: SavedProject) => {
  if (project.isMusicVideoMode) {
    ...
    const shots = project.musicVideoBreakdown?.sectionBreakdowns?.reduce((sum: number, section: any) => sum + (section.shots?.length || 0), 0) || 0
    return { sections, shots, type: 'music-video' }
  } else {
    const chapters = project.breakdown?.storyStructure?.totalChapters || 0
    const shots = project.breakdown?.chapterBreakdowns?.reduce((sum: number, chapter: any) => sum + (chapter.shots?.length || 0), 0) || 0
    const additionalShots = Object.values(project.additionalShots || {}).reduce((sum: number, shots: any) => sum + (shots?.length || 0), 0)
            ...
            {currentProject.isMusicVideoMode ? (
              <>
                {currentProject.musicVideoData?.songTitle && (
                  <div><strong>Song:</strong> {currentProject.musicVideoData.songTitle}</div>
                )}
                {currentProject.musicVideoData?.artist && (
                  <div><strong>Artist:</strong> {currentProject.musicVideoData.artist}</div>
                )}
                <div><strong>Status:</strong> {currentProject.musicVideoBreakdown ? 'Generated' : 'Draft'}</div>
              </>
            ) : (
              <>
                <div><strong>Length:</strong> {currentProject.story?.length || 0} characters</div>
                <div><strong>Status:</strong> {currentProject.breakdown ? 'Generated' : 'Draft'}</div>
              </>
            )}
            <div><strong>Saved:</strong> {currentProjectId ? 'Yes' : 'No'}</div>
          </div>
        </div>
      )}
                    ...
                    {project.isMusicVideoMode ? (
                      <>
                        {project.musicVideoData?.songTitle && project.musicVideoData?.artist && (
                          <div>"{project.musicVideoData.songTitle}" by {project.musicVideoData.artist}</div>
                        )}
                        {project.musicVideoData?.lyrics && (
                          <div className="truncate mt-1">
                            {project.musicVideoData.lyrics.substring(0, 100)}...
                          </div>
                        )}
                      </>
                    ) : (
