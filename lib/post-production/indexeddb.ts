...
class IndexedDBManager {
  ...
  async init(): Promise<void> {
    return new Promise((resolve, reject) => {
      ...
      request.onupgradeneeded = (event) => {
        ...
        // Create cachedVideos store for downloaded videos
        if (!db.objectStoreNames.contains("cachedVideos")) {
          const cachedStore = db.createObjectStore("cachedVideos", { keyPath: "id" });
  ...
  async addReference(id: string, thumbBlob: Blob, fullBlob: Blob, tags: string[], category?: string): Promise<void> {
    if (!this.db) throw new Error("Database not initialized");
    const data = { id, thumbBlob, fullBlob, tags, category: category || 'uncategorized' };
    return new Promise((resolve, reject) => {
  ...
  async getAllReferences(): Promise<Array<{ id: string; thumbBlob: Blob; fullBlob: Blob; tags: string[]; category?: string }>> {
    if (!this.db) throw new Error("Database not initialized");
    return new Promise((resolve, reject) => {
  ...
  async updateReference(id: string, updates: { tags?: string[]; category?: string }): Promise<void> {
    if (!this.db) throw new Error("Database not initialized");
    
    return new Promise(async (resolve, reject) => {
      // First get the existing reference
      const tx1 = this.db!.transaction(["referenceLibrary"], "readonly");
      const store1 = tx1.objectStore("referenceLibrary");
      const getReq = store1.get(id);
      
      getReq.onsuccess = () => {
        const existing = getReq.result;
        if (!existing) {
          reject(new Error("Reference not found"));
          return;
        }
        ...
        const tx2 = this.db!.transaction(["referenceLibrary"], "readwrite");
        const store2 = tx2.objectStore("referenceLibrary");
        const putReq = store2.put(updated);
